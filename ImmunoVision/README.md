...
- The Municipality Map draws the real barangay boundaries of San Jacinto,
  derived from the **Philippine Statistics Authority PSGC 2023** administrative
  shapefiles:
  - `static/data/san_jacinto_barangays.geojson` — lat/lon polygons overlaid on
    the OpenStreetMap base map (Leaflet is vendored in
    `static/js/vendor/leaflet/`, so no CDN and no API key are needed).
  - `static/data/san_jacinto_map.json` — the same boundaries pre-projected to
    SVG paths, used automatically as an **offline fallback** if the base-map
    tiles cannot be reached. The map therefore always renders, with or without
...
static/data/           Barangay boundary geometry (PSA PSGC 2023)
